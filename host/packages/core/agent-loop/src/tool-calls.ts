 ...
 * valid. A terminal scheduler failure also records error results for every
 * already-recorded `tool/call` before rethrowing, so the session transcript
 * remains a valid provider history (no dangling `tool_calls`).
 ...
 * already-started dispatches, records error results for every started call,
 * and rejects with the first failure.
...
async function runGroup(
  ...
  try {
    ...
    // Pair every recorded tool/call before rethrowing. Without this, the
    // provider rejects every later turn with "insufficient tool messages".
    while (committed < started) {
      // oxlint-disable-next-line typescript/no-non-null-assertion -- bounded by started
      const call = group[committed]!
      // oxlint-disable-next-line typescript/no-non-null-assertion -- set in startCall before prepare
      ...
      if (slot !== undefined) {
        ...
        appendToolResult(session, turn, step, call.block, {
          content: [{
            type: 'text',
            text: 'Error: tool call failed during scheduling before a result was recorded',
          }],
          isError: true,
          error: {
            message: 'tool call failed during scheduling',
            info: { name: 'ToolSchedulerError', code: 'TOOL_SCHEDULER_FAILURE' },
          },
        }, callSeq)
      }
      committed++
