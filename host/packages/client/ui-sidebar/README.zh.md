...
`SidebarRootComponentProps` 组合布局 owner share（包括当前 `primaryPage`）、全局 `useSessions` 和 `useWorkspaces` 钩子、已声明的品牌、`sidebar.primary.action`、`sidebar.workspaces` 与 `sidebar.settings` 子 slot，以及注入的 `startSession` 与侧边栏切换回调。这里没有插件 store。
