...
复用 `local_files.py`（列表/限流读）+ 引擎 LLM 出报告。  
SPA：路径输入；工具：`mes_code_review`。

### 5.3 P0-2 人触发提交

复用 `batch_commit` / `git_commit` / `commit_gate`（可先关 ide/skill 门禁）。  
依赖 P0-1 的 `synced_files` 账本。  
SPA「待确认提交」页 → `commit-confirm` **直执**。
...
数据目录：`engine/data/local_dev/`（jobs/sandboxes）、后续 `code_review/`、`automations/`。
