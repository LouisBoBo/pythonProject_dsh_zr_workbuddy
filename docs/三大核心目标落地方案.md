...
> 权威摘要见根目录 `AGENTS.md` §0；Agent 强制规则：`.cursor/rules/workbuddy-ops-standard.mdc`。

| 标准 | 一句话 |
|------|--------|
| **架构保留** | 留唯一 Bridge + `features/`；禁止删 Bridge、禁止业务再进 profile |
| **宿主可不上** | 本机/服务器默认当未装 DSH；业务路径不要求重启 DSH |
| **验收看引擎** | 引擎网页 / 探活；`auto_restart_bridge=false`；无 profile 跳过宿主收尾不算失败 |
| **第三方进 features** | 安装与启停走引擎侧，不依赖是否在跑 DSH |

**边界**：Cordis「对话热挂工具」属 DSH 进程能力；WorkBuddy 产品面是引擎应用。两者共享 `features/` 契约，操作面不绑死宿主。
...
> **已落地（产品文档 P2）**：见 [功能实现/P2-第三方插件安装使用功能实现.md](./功能实现/P2-第三方插件安装使用功能实现.md)（上表阶段 A/B 对应文档内「阶段 A/B」；上表「P2 git…」为后续增强，勿与文件名混淆）。
