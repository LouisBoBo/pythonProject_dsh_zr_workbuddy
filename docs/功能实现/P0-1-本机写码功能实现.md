...
> **删除菜单**：同车道增强，见专篇 [P0-1-删除菜单车道.md](./P0-1-删除菜单车道.md)  
> **不包含**：自动 Git 提交（P0-2）、审码（P0-3）、定时自动化（P0-4）；**写码结论与删除车道对齐暂缓**

---

## 1. 一句话总结（给非技术人员）

用户在 WorkBuddy（引擎网页）里说出「要在某台电脑上的项目里改代码 / 做页面」，系统**不会立刻改文件**，而是先像产品经理一样**问清楚、出选项、出确认卡**；用户点「确认并用 Cursor 写入本机」后，后台在**隔离副本（沙箱）**里让 Cursor AI 改代码，改完再把**允许范围内的文件**同步回真实项目目录，并在界面里**逐步显示进度**。全程**不会自动提交 Git**。

若诉求是「删除某某菜单子项」，走同一工具卡，但 Cursor 只读定位、引擎改盘与结论汇总，详见删除专篇。
...
```text
┌─────────────────────────────────────────────────────────────┐
│  引擎 SPA（主入口）+ mes-bridge（热插拔宿主）（client.js）                      │
│  · 展示思考 / 选项卡 / 确认卡 / 进度卡                        │
...
> **体验总方案（接 Cursor、适配 DSH 壳，不另造编码平台）**：见 [AI-Coding接入Cursor-体验逼近方案.md](./AI-Coding接入Cursor-体验逼近方案.md)（阶段 A 续改 → B/C 审清单+deferred → D 过程流 → E 跑中追问视 SDK）。

| 项 | 说明 |
| --- | --- |
| **P0-2 提交确认** | 基于 `synced_files` 账本，SPA「待确认提交」→ 人点后再 git commit |
| **Job 历史页** | 引擎 SPA 列出 `local_dev/jobs`，可复查同步文件与错误 |
| **续改短句** | 同一 workspace 带上 job_id 上下文，减少重复收集 → **归入体验方案阶段 A** |
...
| **范围外变更 SPA 卡** | → **体验方案阶段 B+C**（pending_review + apply HITL） |
| **模块关键词库可配置** | `MODULE_HINTS` 进配置中心，减少硬编码 |
| **Cursor 进度细粒度** | → **体验方案阶段 D**（tool_events + 可控代码折叠） |
...
完整话术见 [功能迁移-写码审码提交自动化.md](../架构与选型/功能迁移-写码审码提交自动化.md) §9。
...
| 文档 | 说明 |
| --- | --- |
| [功能迁移-写码审码提交自动化.md](../架构与选型/功能迁移-写码审码提交自动化.md) | P0 定稿方案与验收 |
