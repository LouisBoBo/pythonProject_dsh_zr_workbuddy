...
> **状态**：已实现（P1 MVP）· **一体部署增强**：`unified_product=true`（默认）一次确认 → 同步单元 → 拉起引擎+聊天壳 → `entry_url` 探活  
> **范围**：本机 SSH + rsync → 预发/staging；按**部署单元**增量同步；人确认后执行  
> **产品验收**：浏览器一体入口可用（聊天壳，体验对齐本机 `:3080`）；引擎随收尾 ensure/restart  
> **不包含**：GitHub Actions、无人确认部署、生产默认可开；不强制同步 Studio / `dsh` 的 `node_modules`

---

## 1. 一句话总结（给非技术人员）

在 WorkBuddy 里说「部署到预发 / 部署上线」时，系统**不会立刻改服务器**，而是先算出这次该同步哪些部分，再弹出**确认卡**。人点确认后，用本机私钥经 SSH **只同步改过的部分**，并**自动把远端引擎和聊天壳拉起来**。

成功后给你**一个浏览器入口**（配置里的入口 URL，通常经 nginx 进聊天页），效果尽量跟本机打开 `:3080` 一样。

**设计原则：一个项目、一次部署、一个入口；改哪发哪只是少传文件。**
