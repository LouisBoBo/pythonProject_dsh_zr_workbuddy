...
> 能力线服从 [三大核心目标落地方案](./三大核心目标落地方案.md)（热插拔 / 增量部署 / 第三方 features）。

> **状态**：已定稿（决策已确认）  
> **约束**：不动 simplified-workbuddy 源码；不动本仓架构铁律（见根目录 `AGENTS.md`）  
> **落地纪律**：**P0 按条复制，每条由人工确认通过后再做下一条**，避免大面积回归。  
> **近况（2026-09-09）**：删除菜单车道已打磨（过程=Cursor、结论=引擎落盘）；写码结论对齐暂缓。专篇见 [功能实现/P0-1-删除菜单车道.md](../功能实现/P0-1-删除菜单车道.md)。知识库分类索引见 [docs/README.md](../README.md)。
...
| 5 | 部署 | **P1 已落地**：按单元增量/全量 + SSH（G2）；业务成败看引擎（见 `docs/功能实现/P1-自动化部署-按单元增量.md`、三大核心目标） |
...
| 能力 | 状态 | 说明 |
|------|------|------|
| 按单元增量/全量部署（SSH） | **已落地** | `code-deploy`；改哪发哪；引擎重启/探活为业务成功；bridge 默认仅同步 |
...
- 本仓：`AGENTS.md`、`docs/产品与口径/目录结构与用法说明.md`  
